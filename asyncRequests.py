...
async def user_chat_id(chat_id):
    async with aiohttp.ClientSession() as session:
        async with session.get(url='https://cashback.eastline.uz/tgid', params={'tgUserId': chat_id}) as response:
            return await response.json()


async def contact_create(chat_id, number):
    async with aiohttp.ClientSession() as session:
        async with session.post(url='https://cashback.eastline.uz/user/myapp/', data={'tgUserId': chat_id, 'phone': number}) \
                as response:
            return await response.json()
...
async def check_invoice(invoice_id):
    async with aiohttp.ClientSession() as session:
        async with session.get(url='https://cashback.eastline.uz/invoice', params=invoice_id) as response:
            return await response.json()
...
async def invoice_create(chat_id, number):
    async with aiohttp.ClientSession() as session:
        async with session.post(url='https://cashback.eastline.uz/user/coolapp/', data={'UserId': chat_id, 'invoiceId': number}) \
                as response:
            return await response.json()
